...
nndeploy-app 是独立的 Workflow 编排容器，提供可视化 WebUI 编辑推理 workflow。xclabel-server 训练完成的模型可手动发布到 nndeploy-app，用户在 WebUI 中基于这些模型创建和编辑 workflow，deploy 容器可下载并执行这些 workflow。

**架构：**
```
xclabel-server (Flask:5000)          nndeploy-app (8002)
    |                                      |
    |  手动发布模型                         |  WebUI 编辑 workflow
    |  resources/models/*.onnx             |  resources/workflow/*.json
    v                                      v
projects/<project>/models/*.pt    ←── 共享 resources 卷
```

**模型发布流程：**

1. 在 xclabel-server 中完成 YOLO 训练并导出 ONNX
2. 进入训练页面，在模型版本列表中点击"发布"按钮
3. 确认后，ONNX 模型被复制到 `resources/models/<project>_<version>.onnx`
4. 打开 nndeploy-app WebUI（http://localhost:8002）
5. 在 WebUI 中选择已发布的模型，创建/编辑 workflow

**nndeploy-app WebUI 访问：**
```
http://localhost:8002
```

**共享资源卷：**
- `resources/models/`：xclabel 发布的模型文件
- `resources/workflow/`：nndeploy-app 生成的 workflow JSON
- `resources/db/nndeploy.db`：nndeploy-app 的 SQLite 数据库
...
| nndeploy-app | 8002 | Workflow 编排服务 |
...
├── resources/                # nndeploy-app 共享资源目录（运行时自动创建）
│   ├── models/               # 已发布的 ONNX 模型
│   ├── workflow/             # nndeploy-app 生成的 workflow JSON
│   ├── images/               # 图片资源
│   ├── videos/               # 视频资源
│   ├── audios/               # 音频资源
│   ├── db/                   # nndeploy-app SQLite 数据库
│   └── template/             # nndeploy-app 内置模板
